...
const API_BASE_URL =
  process.env.API_BASE_URL || "https://de-inherit.vercel.app";
...
async function main() {
  ...
  try {
    ...
    const pulseResponse = await fetch(
      `${API_BASE_URL}/api/check-pulse?wallet=${ownerWallet}`,
    );
    ...
    if (pulseData.isDead && !pulseData.isReleased) {
      console.log(
        "🔴 User is inactive beyond threshold. Releasing inheritance...",
      );
